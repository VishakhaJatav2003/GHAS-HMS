...
### Step 1 — @w1-fetcher
Run `fetch_alerts.sh` via Git Bash. It fetches Dependabot, Code Scanning, and Secret Scanning alerts and writes them to `github_alerts.csv`.

Capture from its output:
- `CSV_PATH` — full path to the generated file: `C:\Users\TanishqShrivas\DummyProj\GHAS-dummy-projects\HMS\github_alerts.csv`

If the file is not confirmed, resolve the latest timestamped file with:
```
...
### Step 2 — @w1-sorter
Pass `CSV_PATH` from Step 1.

Capture from its output:
- `GROUPED_ALERTS` — dict of service → list of alerts (all types)
...
║  Excel report         : github_alerts.csv            ║
╚══════════════════════════════════════════════════════╝
```
...
- Always resolve `CSV_PATH` explicitly before passing it to subsequent sub-agents
